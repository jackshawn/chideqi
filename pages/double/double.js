...
Page({
	...
	handleQuery: function (cbMei, cbEle, isSecond) {
		var _this = this;
		var sn = (function () {
			var r, from, to;
			var url = isSecond ? _this.data.url1 : _this.data.url;
			if (url.indexOf('meituan') !== -1) {
				...
				// luckuy num
				var ln = url.slice(url.indexOf('&lucky_number=') + 14, url.indexOf('&track_id'));
				if (isSecond) {
					_this.setData({
						luckyNum1: ln
					})
